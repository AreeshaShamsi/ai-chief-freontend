---
source_file: "temp-graphify/graphify/build.py"
type: "code"
community: "graphify/build.py"
location: "L536"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/graphify/buildpy
---

# graph_has_legacy_ids()

## Connections
- [[Path_2]] - `references` [EXTRACTED]
- [[Whether a loaded graph still uses pre-1504 node IDs (parent-dir  filename…]] - `rationale_for` [EXTRACTED]
- [[_file_stem()]] - `calls` [EXTRACTED]
- [[_load_graph()]] - `calls` [EXTRACTED]
- [[_norm_source_file()]] - `calls` [EXTRACTED]
- [[_old_file_stems()]] - `calls` [EXTRACTED]
- [[cli.py]] - `imports` [EXTRACTED]
- [[dispatch_command()]] - `calls` [EXTRACTED]
- [[graphifybuild.py]] - `contains` [EXTRACTED]
- [[make_id()]] - `calls` [EXTRACTED]
- [[normalize_id()]] - `calls` [EXTRACTED]
- [[serve.py]] - `imports` [EXTRACTED]
- [[test_build.py]] - `imports` [EXTRACTED]
- [[test_graph_has_legacy_ids_detects_old_scheme()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/graphify/buildpy