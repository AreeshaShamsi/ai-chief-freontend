---
source_file: "temp-graphify/tests/test_chunking.py"
type: "code"
community: "test_chunking.py"
location: "L332"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_chunkingpy
---

# test_truncated_chunk_is_cached_partial_and_missed_on_reload()

## Connections
- [[A single-file chunk that stays truncated is checkpointed as a PARTIAL entry, so…]] - `rationale_for` [EXTRACTED]
- [[_extraction_system()]] - `calls` [EXTRACTED]
- [[extract_corpus_parallel()]] - `calls` [EXTRACTED]
- [[load_cached()]] - `calls` [EXTRACTED]
- [[test_chunking.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_chunkingpy