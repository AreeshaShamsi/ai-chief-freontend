---
source_file: "temp-graphify/tests/test_languages.py"
type: "code"
community: "extract_objc"
location: "L1419"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/extract_objc
---

# test_objc_dot_syntax_unresolvable_property_zero_edges()

## Connections
- [[Accessing a property not defined in the current class produces zero accesses…]] - `rationale_for` [EXTRACTED]
- [[extract_objc()]] - `calls` [INFERRED]
- [[test_languages.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/extract_objc