---
source_file: "temp-graphify/graphify/cache.py"
type: "code"
community: "cache.py"
location: "L853"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/cachepy
---

# save_cached()

## Connections
- [[Path_3]] - `references` [EXTRACTED]
- [[Save extraction result for this file. Stores as graphify-…]] - `rationale_for` [EXTRACTED]
- [[_extract_sequential()]] - `calls` [EXTRACTED]
- [[_extract_single_file()]] - `calls` [EXTRACTED]
- [[_relativize_ids_in()]] - `calls` [EXTRACTED]
- [[_relativize_source_files_in()]] - `calls` [EXTRACTED]
- [[_resolve_prompt_fp()]] - `calls` [EXTRACTED]
- [[cache.py]] - `contains` [EXTRACTED]
- [[cache_dir()]] - `calls` [EXTRACTED]
- [[extract.py]] - `imports` [EXTRACTED]
- [[file_hash()]] - `calls` [EXTRACTED]
- [[save_semantic_cache()]] - `calls` [EXTRACTED]
- [[test_ast_cache_invalidated_on_version_bump()]] - `calls` [EXTRACTED]
- [[test_ast_cache_version_bump_cleans_stale_entries()]] - `calls` [EXTRACTED]
- [[test_cache.py]] - `imports` [EXTRACTED]
- [[test_cache_miss_on_change()]] - `calls` [EXTRACTED]
- [[test_cache_portable_across_roots()]] - `calls` [EXTRACTED]
- [[test_cache_roundtrip()]] - `calls` [EXTRACTED]
- [[test_cached_files()]] - `calls` [EXTRACTED]
- [[test_cached_ids_round_trip_under_the_same_root()]] - `calls` [EXTRACTED]
- [[test_clear_cache()]] - `calls` [EXTRACTED]
- [[test_detect.py]] - `imports` [EXTRACTED]
- [[test_load_cached_absolutizes_source_file()]] - `calls` [EXTRACTED]
- [[test_relative_root_does_not_reanchor_an_already_canonical_id()]] - `calls` [EXTRACTED]
- [[test_save_cached_in_root_symlink_keeps_symlink_name()]] - `calls` [EXTRACTED]
- [[test_save_cached_relativizes_source_file()]] - `calls` [EXTRACTED]
- [[test_save_manifest_skips_semantic_hash_for_files_without_cache()]] - `calls` [EXTRACTED]
- [[test_semantic_cache_survives_version_bump()]] - `calls` [EXTRACTED]
- [[test_semantic_prune_handles_deleted_file()]] - `calls` [EXTRACTED]
- [[test_semantic_prune_ignores_ast_and_tmp()]] - `calls` [EXTRACTED]
- [[test_semantic_prune_keeps_live_unchanged_entries()]] - `calls` [EXTRACTED]
- [[test_semantic_prune_removes_orphan_entries()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/cachepy