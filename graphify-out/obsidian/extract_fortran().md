---
source_file: "temp-graphify/graphify/extractors/fortran.py"
type: "code"
community: "test_languages.py"
location: "L44"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/test_languagespy
---

# extract_fortran()

## Connections
- [[Extract programs, modules, subroutines, functions, use statements, and calls…]] - `rationale_for` [EXTRACTED]
- [[Path_20]] - `references` [EXTRACTED]
- [[_cpp_preprocess()]] - `calls` [EXTRACTED]
- [[_file_stem()]] - `calls` [EXTRACTED]
- [[_make_id()]] - `calls` [EXTRACTED]
- [[_read_text()]] - `calls` [EXTRACTED]
- [[e()]] - `indirect_call` [INFERRED]
- [[extract.py]] - `indirect_call` [INFERRED]
- [[extractors__init__.py]] - `indirect_call` [INFERRED]
- [[fortran.py]] - `contains` [EXTRACTED]
- [[test_fortran_capital_F_parses_preprocessed()]] - `calls` [INFERRED]
- [[test_fortran_case_insensitive_names()]] - `calls` [INFERRED]
- [[test_fortran_finds_calls()]] - `calls` [INFERRED]
- [[test_fortran_finds_derived_type()]] - `calls` [INFERRED]
- [[test_fortran_finds_function()]] - `calls` [INFERRED]
- [[test_fortran_finds_function_call()]] - `calls` [INFERRED]
- [[test_fortran_finds_module()]] - `calls` [INFERRED]
- [[test_fortran_finds_program()]] - `calls` [INFERRED]
- [[test_fortran_finds_subroutines()]] - `calls` [INFERRED]
- [[test_fortran_finds_use_imports()]] - `calls` [INFERRED]
- [[test_fortran_no_dangling_edges()]] - `calls` [INFERRED]
- [[test_fortran_parameter_and_return_type_contexts()]] - `calls` [INFERRED]
- [[test_fortran_use_edges_have_use_context()]] - `calls` [INFERRED]

#graphify/code #graphify/INFERRED #community/test_languagespy