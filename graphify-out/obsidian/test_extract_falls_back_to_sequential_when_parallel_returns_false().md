---
source_file: "temp-graphify/tests/test_extract.py"
type: "code"
community: "test_extract.py"
location: "L1542"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_extractpy
---

# test_extract_falls_back_to_sequential_when_parallel_returns_false()

## Connections
- [[extract()]] - `calls` [EXTRACTED]
- [[extract() must run sequential when _extract_parallel signals failure (returns…]] - `rationale_for` [EXTRACTED]
- [[test_extract.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_extractpy