---
source_file: "temp-graphify/graphify/export.py"
type: "code"
community: "to_json"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/to_json
---

# export.py

## Connections
- [[_cap_filename()]] - `contains` [EXTRACTED]
- [[_cypher_escape()]] - `contains` [EXTRACTED]
- [[_cypher_label()]] - `contains` [EXTRACTED]
- [[_dedup_node_filenames()]] - `contains` [EXTRACTED]
- [[_git_head()]] - `contains` [EXTRACTED]
- [[_node_community_map()]] - `imports` [EXTRACTED]
- [[_obsidian_safe_stem()]] - `contains` [EXTRACTED]
- [[_obsidian_tag()]] - `contains` [EXTRACTED]
- [[_strip_diacritics()]] - `contains` [EXTRACTED]
- [[_yaml_str()]] - `contains` [EXTRACTED]
- [[attach_hyperedges()]] - `contains` [EXTRACTED]
- [[backup_if_protected()]] - `contains` [EXTRACTED]
- [[check_graph_file_size_cap()]] - `imports` [EXTRACTED]
- [[edge_data()]] - `imports` [EXTRACTED]
- [[existing_graph_node_count()]] - `contains` [EXTRACTED]
- [[exportersbase.py]] - `imports_from` [EXTRACTED]
- [[graphdb.py]] - `imports_from` [EXTRACTED]
- [[graphifyanalyze.py]] - `imports_from` [EXTRACTED]
- [[graphifybuild.py]] - `imports_from` [EXTRACTED]
- [[html.py]] - `imports_from` [EXTRACTED]
- [[prune_dangling_edges()]] - `contains` [EXTRACTED]
- [[push_to_falkordb()]] - `imports` [EXTRACTED]
- [[push_to_neo4j()]] - `imports` [EXTRACTED]
- [[sanitize_label()]] - `imports` [EXTRACTED]
- [[security.py]] - `imports_from` [EXTRACTED]
- [[test_confidence.py]] - `imports_from` [EXTRACTED]
- [[test_cross_extension_reexport_self_cycle.py]] - `imports_from` [EXTRACTED]
- [[test_export.py]] - `imports_from` [EXTRACTED]
- [[test_hypergraph.py]] - `imports_from` [EXTRACTED]
- [[test_obsidian_dangling_member.py]] - `imports_from` [EXTRACTED]
- [[test_obsidian_filename_cap.py]] - `imports_from` [EXTRACTED]
- [[test_pipeline.py]] - `imports_from` [EXTRACTED]
- [[to_canvas()]] - `contains` [EXTRACTED]
- [[to_cypher()]] - `contains` [EXTRACTED]
- [[to_graphml()]] - `contains` [EXTRACTED]
- [[to_html()]] - `indirect_call` [INFERRED]
- [[to_json()]] - `contains` [EXTRACTED]
- [[to_obsidian()]] - `contains` [EXTRACTED]
- [[to_svg()]] - `contains` [EXTRACTED]
- [[write_json_atomic()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/to_json