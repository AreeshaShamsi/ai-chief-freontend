---
source_file: "temp-graphify/tests/test_symbol_resolution.py"
type: "code"
community: "test_symbol_resolution.py"
location: "L127"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_symbol_resolutionpy
---

# test_resolve_cross_file_raw_calls_real_edge_survives_test_mock()

## Connections
- [[A real cross-file call must resolve to the SRC definition even when a same-…]] - `rationale_for` [EXTRACTED]
- [[resolve_cross_file_raw_calls()]] - `calls` [EXTRACTED]
- [[test_symbol_resolution.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_symbol_resolutionpy