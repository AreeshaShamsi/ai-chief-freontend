---
source_file: "temp-graphify/tests/test_semantic_similarity.py"
type: "code"
community: "test_semantic_similarity.py"
location: "L43"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_semantic_similaritypy
---

# _make_two_edge_graph()

## Connections
- [[Graph with one semantically_similar_to edge and one references edge, both…]] - `rationale_for` [EXTRACTED]
- [[test_semantic_edge_reason_mentions_similarity()]] - `calls` [EXTRACTED]
- [[test_semantic_edge_scores_higher_than_references()]] - `calls` [EXTRACTED]
- [[test_semantic_similarity.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_semantic_similaritypy