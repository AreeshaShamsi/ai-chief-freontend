---
source_file: "temp-graphify/tests/test_stat_index_portability.py"
type: "code"
community: "test_stat_index_portability.py"
location: "L124"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_stat_index_portabilitypy
---

# test_legacy_absolute_index_migrates_gracefully()

## Connections
- [[A pre-2199 index keyed by absolute paths still HITS on the unmoved root, and…]] - `rationale_for` [EXTRACTED]
- [[_count_read_bytes()]] - `calls` [EXTRACTED]
- [[_flush_stat_index()]] - `calls` [EXTRACTED]
- [[_read_index()]] - `calls` [EXTRACTED]
- [[_reset_stat_index()_2]] - `calls` [EXTRACTED]
- [[_stat_index_path()]] - `calls` [EXTRACTED]
- [[file_hash()]] - `calls` [EXTRACTED]
- [[test_stat_index_portability.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_stat_index_portabilitypy