---
source_file: "temp-graphify/graphify/install.py"
type: "code"
community: "graphify/__main__.py"
location: "L2013"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/graphify/__main__py
---

# dispatch_install_cli()

## Connections
- [[Handle `graphify install-command` dispatch (installuninstall + every per-…]] - `rationale_for` [EXTRACTED]
- [[Path_42]] - `calls` [EXTRACTED]
- [[_agents_install()]] - `calls` [EXTRACTED]
- [[_agents_platform_install()]] - `calls` [EXTRACTED]
- [[_agents_platform_uninstall()]] - `calls` [EXTRACTED]
- [[_agents_uninstall()]] - `calls` [EXTRACTED]
- [[_amp_install()]] - `calls` [EXTRACTED]
- [[_amp_uninstall()]] - `calls` [EXTRACTED]
- [[_antigravity_install()]] - `calls` [EXTRACTED]
- [[_antigravity_uninstall()]] - `calls` [EXTRACTED]
- [[_cursor_install()]] - `calls` [EXTRACTED]
- [[_cursor_uninstall()]] - `calls` [EXTRACTED]
- [[_kilo_install()]] - `calls` [EXTRACTED]
- [[_kilo_uninstall()]] - `calls` [EXTRACTED]
- [[_kiro_install()]] - `calls` [EXTRACTED]
- [[_kiro_uninstall()]] - `calls` [EXTRACTED]
- [[_print_install_usage()]] - `calls` [EXTRACTED]
- [[_project_install()]] - `calls` [EXTRACTED]
- [[_project_uninstall()]] - `calls` [EXTRACTED]
- [[_project_uninstall_all()]] - `calls` [EXTRACTED]
- [[_remove_skill_file()]] - `calls` [EXTRACTED]
- [[_run_cli()]] - `calls` [EXTRACTED]
- [[_uninstall_codex_hook()]] - `calls` [EXTRACTED]
- [[claude_install()]] - `calls` [EXTRACTED]
- [[claude_uninstall()]] - `calls` [EXTRACTED]
- [[codebuddy_install()]] - `calls` [EXTRACTED]
- [[codebuddy_uninstall()]] - `calls` [EXTRACTED]
- [[gemini_install()]] - `calls` [EXTRACTED]
- [[gemini_uninstall()]] - `calls` [EXTRACTED]
- [[graphify__main__.py]] - `imports` [EXTRACTED]
- [[install()_1]] - `calls` [EXTRACTED]
- [[install.py]] - `contains` [EXTRACTED]
- [[uninstall_all()]] - `calls` [EXTRACTED]
- [[vscode_install()]] - `calls` [EXTRACTED]
- [[vscode_uninstall()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/graphify/__main__py