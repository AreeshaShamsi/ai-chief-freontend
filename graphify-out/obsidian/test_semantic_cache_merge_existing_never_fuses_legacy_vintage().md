---
source_file: "temp-graphify/tests/test_cache.py"
type: "code"
community: "save_semantic_cache"
location: "L1241"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/save_semantic_cache
---

# test_semantic_cache_merge_existing_never_fuses_legacy_vintage()

## Connections
- [[load_cached()]] - `calls` [EXTRACTED]
- [[merge_existing must not union a pre-fingerprint entry into a write it is about…]] - `rationale_for` [EXTRACTED]
- [[save_semantic_cache()]] - `calls` [EXTRACTED]
- [[test_cache.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/save_semantic_cache