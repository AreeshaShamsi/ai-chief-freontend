...
function ScoreBadge({ score }) {
    const m = { hot: { l: "🔥 Hot", bg: C.hotBg, c: C.hot, b: C.hotBdr }, warm: { l: "⚡ Warm", bg: C.warmBg, c: C.warm, b: C.warmBdr }, cold: { l: "❄️ Cold", bg: "#F8FAFC", c: C.muted, b: C.border }, invalid: { l: "✕ Invalid", bg: "#F9FAFB", c: "#9CA3AF", b: C.border } }[score] || { l: "–", bg: "#F8FAFC", c: C.muted, b: C.border };
...
function Btn({ children, primary, sm, danger, style, onClick, disabled }) {
    return <button onClick={onClick} disabled={disabled}
        style={{ background: danger ? "#FEF2F2" : primary ? C.accent : "transparent", color: danger ? C.hot : primary ? "#fff" : C.muted, border: `1px solid ${danger ? C.hot : primary ? C.accent : C.border}`, borderRadius: 7, padding: sm ? "5px 12px" : "8px 18px", fontSize: sm ? 11 : 13, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? .5 : 1, ...style }}>{children}</button>;
}

export { ScoreBadge, Avatar, Waveform, Card, Btn, C };