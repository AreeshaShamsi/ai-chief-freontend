...
function Stepper({ active }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 0, padding: "18px 28px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step.n} style={{ display: "flex", alignItems: "center", flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 0 }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: isCurrent || isDone ? C.accent : C.surface, border: `2px solid ${isCurrent || isDone ? C.accent : C.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: T.font.weight.bold, fontSize: T.font.size.bodySmall, color: isCurrent || isDone ? "#fff" : C.muted, flexShrink: 0, transition: "all 0.2s" }}>
                {isDone ? <FiCheck size={13} /> : step.n}
              </div>
              <div style={{ marginTop: 5, textAlign: "center" }}>
                <div style={{ fontSize: T.font.size.bodySmall, fontWeight: isCurrent ? T.font.weight.bold : T.font.weight.medium, color: isCurrent ? C.accent : isDone ? C.text : C.muted, whiteSpace: "nowrap" }}>
                  {step.label}
                </div>
                <div style={{ fontSize: T.font.size.caption, color: C.muted, whiteSpace: "nowrap" }}>{step.sub}</div>
              </div>
            </div>
            {i < STEPS.length - 1 && (
              <div style={{ flex: 1, height: 2, background: isDone ? C.accent : C.border, margin: "0 8px", marginBottom: 26, transition: "background 0.2s" }} />
            )}
          </div>
        );
      })}
...
const SETUP_STEPS = [
  {
    n: 1,
    title: "Go to your MagicBricks Account",
    desc: "Login to your MagicBricks Builder / Broker account.",
  },
  {
    n: 2,
    title: "Request CRM Integration",
    desc: "Contact your MagicBricks Account Manager and request CRM / API integration.",
  },
  {
    n: 3,
    title: "Provide Webhook URL",
    desc: "Share the webhook URL below with your MagicBricks Account Manager.",
  },
  {
    n: 4,
...
const WEBHOOK_URL = "https://crm.aichief.in/api/webhooks/magicbricks";

// ─── Modal ────────────────────────────────────────────────────────────────────
export default function MagicBricksConnectorModal({ onClose }) {
      ...
      <div style={{ display: "flex", flexDirection: "column", maxHeight: "90vh", background: C.card, borderRadius: T.radius.card, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "20px 28px 16px", borderBottom: `1px solid ${C.border}`, flexShrink: 0, gap: 20 }}>
          <div style={{ display: "flex", gap: 18, alignItems: "flex-start", flex: 1, minWidth: 0 }}>
            <img src={magicbricksLogo} alt="MagicBricks logo" style={{ width: 130, objectFit: "contain", flexShrink: 0, marginTop: 4 }} />
            <div>
              <h2 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: T.font.weight.extraBold, color: C.text, lineHeight: 1.2 }}>
                Integrate with MagicBricks
              </h2>
              <p style={{ margin: "0 0 10px", color: C.muted, fontSize: T.font.size.bodySmall, lineHeight: 1.5 }}>
                Connect your MagicBricks account to automatically capture new leads<br />into your CRM in real-time.
              </p>
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
          ...
          <div style={{ padding: "24px 28px", borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column" }}>
            <h3 style={{ margin: "0 0 4px", fontSize: T.font.size.cardTitle, fontWeight: T.font.weight.bold, color: C.text }}>
              1. Connect Your MagicBricks Account
            </h3>
            <p style={{ margin: "0 0 20px", color: C.muted, fontSize: T.font.size.bodySmall }}>
              Follow the steps below to connect MagicBricks with your CRM.
            </p>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
                {[
                  ...
                  "Reduce response time and close more deals",
                ].map((b) => (
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10 }}>
                <div style={{ border: `1px solid ${C.border}`, borderRadius: T.radius.md, padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "center", background: C.surface }}>
                  <img src={magicbricksLogo} alt="MagicBricks" style={{ width: 80, objectFit: "contain" }} />
                </div>
...
MagicBricksConnectorModal.propTypes = { onClose: PropTypes.func.isRequired };
