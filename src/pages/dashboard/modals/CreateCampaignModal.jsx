...
function CreateCampaignModal({ onClose, onLaunch }) {
    ...
    const [uploadPhase, setUploadPhase] = useState("idle"); // idle|uploading|mapped
    const [colMap, setColMap] = useState({ phone: "Phone", name: "Name", project: "Project", budget: "Budget" });
    ...
    const triggerUpload = () => {
        ...
        setTimeout(() => setUploadPhase("mapped"), 1400);
    };

    const handleLaunch = () => {
        setLaunching(true);
        setTimeout(() => {
            setLaunching(false); setLaunched(true);
            setTimeout(() => { onLaunch(name, script); }, 1800);
        }, 1800);
    };
                ...
                <div style={{ padding: "18px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ display: "flex", gap: 5 }}>
                            {[1, 2].map(s => <div key={s} style={{ width: s === step ? 20 : 6, height: 6, borderRadius: 3, background: s <= step ? C.accent : C.border, transition: "all .2s" }} />)}
                        </div>
                    ...
                    <div style={{ padding: "22px 24px" }}>
                        {uploadPhase === "idle" && (
                            <>
                                <div onClick={triggerUpload}
                                    style={{ border: `2px dashed ${C.border}`, borderRadius: 12, padding: "36px 24px", textAlign: "center", cursor: "pointer", transition: "border-color .15s", marginBottom: 18 }}
                                ...
                                <div style={{ background: "#FFFBEB", borderRadius: 8, padding: "10px 14px", fontSize: 11, color: "#92400E", border: `1px solid ${C.warmBdr}` }}>
                                    💡 Required columns: phone number and lead name. All other columns are optional.
                                </div>
                            </>
                        )}
                        ...
                        {uploadPhase === "uploading" && (
                            <div style={{ textAlign: "center", padding: "40px 24px" }}>
                                ...
                                <div style={{ fontSize: 12, color: C.muted }}>Detecting columns and validating phone numbers</div>
                                <div style={{ marginTop: 18, height: 4, background: C.border, borderRadius: 2, overflow: "hidden" }}>
                        ...
                        {uploadPhase === "mapped" && !launched && (
                            <>
                                {/* File success strip */}
                                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 8, background: C.greenBg, border: `1px solid ${C.greenBdr}`, marginBottom: 18 }}>
                                    <span style={{ fontSize: 18 }}>✅</span>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontSize: 12, fontWeight: 600, color: "#065F46" }}>leads_dec_batch.xlsx detected</div>
                                        <div style={{ fontSize: 11, color: C.muted }}>320 rows · 6 columns found</div>
                                    </div>
                                    <button onClick={() => setUploadPhase("idle")} style={{ background: "none", border: "none", cursor: "pointer", color: C.muted, fontSize: 12 }}>Change</button>
                                </div>

                                {/* Column mapper */}
                                <div style={{ marginBottom: 16 }}>
                                    <div style={{ fontSize: 12, fontWeight: 600, color: C.text, marginBottom: 10 }}>Map columns</div>
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                                        {[
                                            { label: "Phone number", key: "phone", req: true },
                                            { label: "Lead name", key: "name", req: true },
                                            { label: "Project interest", key: "project", req: false },
                                            { label: "Budget", key: "budget", req: false },
                                        ].map(f => (
                                            <div key={f.key}>
                                                <label style={{ display: "block", fontSize: 11, fontWeight: 500, color: C.text, marginBottom: 4 }}>
                                                    {f.label} {f.req && <span style={{ color: C.hot }}>*</span>}
                                                    {f.req && <span style={{ marginLeft: 4, fontSize: 10, color: C.green, fontWeight: 500 }}>✓ auto-detected</span>}
                                                </label>
                                                <select value={colMap[f.key]} onChange={e => setColMap(m => ({ ...m, [f.key]: e.target.value }))}
                                                    style={{ width: "100%", padding: "7px 10px", border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 12, background: C.card, color: C.text, outline: "none" }}>
                                                    {DETECTED_COLS.map(c => <option key={c}>{c}</option>)}
                                                </select>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div style={{ marginBottom: 18 }}>
                                    <div style={{ fontSize: 11, fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 7 }}>Preview (3 of 320 rows)</div>
                                    <div style={{ border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
                                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                                            <thead>
                                                <tr style={{ background: "#F8FAFC" }}>
                                                    {[colMap.name, colMap.phone, colMap.project, colMap.budget].map(h => (
                                                        <th key={h} style={{ padding: "7px 10px", textAlign: "left", fontWeight: 600, color: C.muted, borderBottom: `1px solid ${C.border}` }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {PREVIEW_ROWS.map((r, i) => (
                                                    <tr key={i} style={{ borderBottom: i < 2 ? `1px solid ${C.border}` : "none" }}>
                                                        <td style={{ padding: "7px 10px", color: C.text, fontWeight: 500 }}>{r.Name}</td>
                                                        <td style={{ padding: "7px 10px", color: C.muted }}>{r.Phone}</td>
                                                        <td style={{ padding: "7px 10px", color: C.muted }}>{r.Project}</td>
                                                        <td style={{ padding: "7px 10px", color: C.muted }}>{r.Budget}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    ...
                                    <Btn primary onClick={handleLaunch} disabled={launching}
                                        style={{ background: C.green, borderColor: C.green, minWidth: 160, textAlign: "center" }}>
                        ...
                        {launched && (
                            <div style={{ textAlign: "center", padding: "32px 24px" }}>
                                ...
                                <div style={{ fontSize: 13, color: C.muted, marginBottom: 14 }}>{name || "New campaign"}</div>
