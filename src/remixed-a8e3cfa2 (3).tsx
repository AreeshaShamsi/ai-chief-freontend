...
function highlight(text,query){
  ...
  if(i===-1) return text;
  return <>{text.slice(0,i)}<mark style={{background:"#FEF08A",borderRadius:2,padding:"0 1px"}}>{text.slice(i,i+query.length)}</mark>{text.slice(i+query.length)}</>;
}
const FILE_ICONS={"pdf":"📄","xlsx":"📊","xls":"📊","doc":"📝","docx":"📝"};
const initInventory=[
  {id:1,name:"Prestige Lakeside",location:"Sarjapur Road, Bengaluru",bhk:["2BHK","3BHK","4BHK"],carpetArea:"1,050 – 2,200 sq.ft.",priceRange:"₹85L – ₹1.8 Cr",possession:"Ready to move",units:24,facing:["East","North"],amenities:"Clubhouse, Pool, Gym, Jogging Track",highlight:"Last few units left. RERA approved.",status:"active"},
  {id:2,name:"Sobha Hartland",location:"Whitefield, Bengaluru",bhk:["2BHK","3BHK"],carpetArea:"820 – 1,450 sq.ft.",priceRange:"₹72L – ₹1.2 Cr",possession:"Dec 2025",units:58,facing:["East","West","North"],amenities:"Rooftop Garden, EV Charging, Kids Play Area",highlight:"Pre-launch pricing available for limited period.",status:"active"},
  {id:3,name:"Brigade Lakefront",location:"Mysuru Road, Bengaluru",bhk:["1BHK","2BHK","3BHK"],carpetArea:"490 – 1,380 sq.ft.",priceRange:"₹45L – ₹95L",possession:"Jun 2026",units:112,facing:["East","South"],amenities:"Lake View, Co-working Space, Gym",highlight:"Investment-friendly. Rental yield ~4.2% p.a.",status:"active"},
];
const initFaqs=[
...
function InventorySection({inventory,setInventory,openEditProp,delProp}){
  ...
  const filtered=inventory.filter(p=>{
    ...
    return !q||p.name.toLowerCase().includes(q)||p.location.toLowerCase().includes(q)||p.bhk.join(" ").toLowerCase().includes(q)||p.priceRange.toLowerCase().includes(q)||(p.carpetArea||"").toLowerCase().includes(q)||p.possession.toLowerCase().includes(q);
  });
  ...
  const triggerUpload=(fname,ftype)=>{
    ...
    const iv=setInterval(()=>{
      ...
      if(step>=4){clearInterval(iv);setPhase("processing");
    ...
    <div>
      ...
      <div style={{marginBottom:14}}>
        <div style={{position:"relative",marginBottom:12}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by project name, location, BHK, price, carpet area…"
            style={{width:"100%",padding:"9px 12px 9px 34px",border:`1.5px solid ${search?C.accent:C.border}`,borderRadius:9,fontSize:12,outline:"none",boxSizing:"border-box",background:C.card,color:C.text,transition:"border-color .15s"}}/>
          {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:C.muted,fontSize:14,lineHeight:1}}>✕</button>}
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{flex:1,height:1,background:C.border,width:40}}/>
            <span style={{fontSize:11,color:C.muted,fontWeight:500}}>
              {search?`${filtered.length} of ${inventory.length} properties`:"Extracted inventory"}
            </span>
            <div style={{flex:1,height:1,background:C.border,width:40}}/>
          </div>
          {search&&filtered.length===0&&<span style={{fontSize:11,color:C.hot}}>No match found</span>}
        </div>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {filtered.map(p=>(
          <Card key={p.id} style={{padding:0,overflow:"hidden"}}>
            <div style={{padding:"13px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style={{flex:1}}><div style={{fontWeight:700,fontSize:13,color:C.text}}>{highlight(p.name,search)}</div><div style={{fontSize:11,color:C.muted,marginTop:1}}>📍 {highlight(p.location,search)}</div></div>
            ...
            <div style={{padding:"13px 18px"}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:9,marginBottom:10}}>
                {[["BHK Types",p.bhk.join(", "),"🛏"],["Carpet Area",p.carpetArea||"—","📐"],["Price Range",p.priceRange,"💰"],["Possession",p.possession,"📅"],["Units Left",p.units,"🏗"]].map(([l,v,ic])=>(
                  <div key={l} style={{background:C.bg,borderRadius:7,padding:"8px 10px"}}>
                    ...
                    <div style={{fontSize:12,fontWeight:600,color:C.text}}>{highlight(String(v),search)}</div>
                  </div>
                ))}
              </div>
        ...
        {filtered.length===0&&search&&(
          <div style={{textAlign:"center",padding:"32px",background:C.card,borderRadius:12,border:`1px solid ${C.border}`}}>
            <div style={{fontSize:28,marginBottom:8}}>🔍</div>
            <div style={{fontWeight:600,fontSize:13,color:C.text,marginBottom:4}}>No properties found</div>
            <div style={{fontSize:12,color:C.muted}}>Try searching by project name, location, BHK type, or price range.</div>
          </div>
        )}
      </div>
    </div>
...
function KnowledgeBaseTab(){
  ...
  const emptyProp={name:"",location:"",bhk:"2BHK, 3BHK",carpetArea:"",priceRange:"",possession:"",units:"",facing:"",amenities:"",highlight:"",status:"active"};
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(10,14,26,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,padding:16}}>
          <div style={{background:C.card,borderRadius:16,width:"100%",maxWidth:540,border:`1px solid ${C.border}`,maxHeight:"90vh",overflow:"auto"}}>
            ...
            <div style={{padding:"20px 24px",display:"flex",flexDirection:"column",gap:14}}>
              {[["name","Project Name","e.g. Prestige Lakeside","text"],["location","Location","e.g. Sarjapur Road, Bengaluru","text"],["bhk","BHK Types (comma separated)","e.g. 2BHK, 3BHK, 4BHK","text"],["carpetArea","Carpet Area (sq. ft.)","e.g. 820 – 1,450 sq.ft.","text"],["priceRange","Price Range","e.g. ₹85L – ₹1.8 Cr","text"],["possession","Possession","e.g. Ready to move / Dec 2025","text"],["units","Units Available","e.g. 24","number"],["facing","Facing Options (comma separated)","e.g. East, North, West","text"],["amenities","Amenities","e.g. Pool, Gym, Clubhouse","text"],["highlight","Agent Highlight","Key selling point the agent should mention on calls","text"]].map(([k,l,ph,t])=>(
